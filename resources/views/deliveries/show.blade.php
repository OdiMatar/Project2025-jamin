...
@section('content')
    <div class="container">
        <h1>Specificatie geleverde producten – {{ $product->Naam }}</h1>
            ...
            <div class="mb-3">
                ...
                <ul class="mb-0">
                    @foreach ($suppliers as $s)
                        <li>
                            {{ $s->Naam }} — {{ $s->ContactPersoon }} — {{ $s->LeverancierNummer }} —
                            {{ $s->Mobiel }}
                        </li>
                    @endforeach
                </ul>
        ...
        <a href="{{ route('leveringen.overzicht', array_filter(['start' => $start ?? null, 'end' => $end ?? null])) }}"
            class="btn btn-secondary">← Terug naar overzicht</a>
    </div>
@endsection
