...
@section('content')
    <style>
        ...
        .btn-back:focus {
            outline: .25rem solid rgba(37, 99, 235, .25);
            outline-offset: 2px
        }
    </style>
    ...
    <div class="container py-4">
        ...
        <h1 class="h4 mb-4">Specificatie geleverde producten</h1>

        @if (request()->query('start') || request()->query('end'))
            <div class="mb-3">
                <strong>Tijdvak:</strong>
                {{ request()->query('start') ? \Illuminate\Support\Carbon::parse(request()->query('start'))->format('d-m-Y') : '–' }}
                t/m
                {{ request()->query('end') ? \Illuminate\Support\Carbon::parse(request()->query('end'))->format('d-m-Y') : '–' }}
            </div>
        @endif
                            ...
                            @foreach ($leveringen as $lev)
                                ...
                                @if ($aantal === 0)
                                    @php
                                        ...
                                        $verwachteDatum = $lev->DatumEerstVolgendeLevering
                                            ? \Illuminate\Support\Carbon::parse($lev->DatumEerstVolgendeLevering)
                                ...
                                @else
                                    <tr>
                                        ...
                                        <td>{{ optional(\Illuminate\Support\Carbon::parse($lev->DatumLevering))->format('d-m-Y') }}
                                        </td>
                                        <td>{{ $lev->Aantal }}</td>
                                        <td>
                                            @php
                                                $volgende = $lev->DatumEerstVolgendeLevering
                                                    ? \Illuminate\Support\Carbon::parse(
                                                        $lev->DatumEerstVolgendeLevering,
                                                    )->format('d-m-Y')
        ...
        <div class="mt-3">
            @if (request()->query('start') || request()->query('end'))
                <a href="{{ route('leveringen.overzicht', array_filter(['start' => request()->query('start'), 'end' => request()->query('end')])) }}"
                    class="btn-back" aria-label="Terug naar overzicht geleverde producten">
